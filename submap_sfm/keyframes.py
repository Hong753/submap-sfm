...
contiguous bumps where the trajectories overlap. So we sample coarse-to-fine:
  ...
  * Match a `max_stride`-subsampled grid of A x B.
  * Repeatedly halve the stride down to `min_stride`, re-testing only the
    neighbourhoods of cells that matched, so off-band regions are only probed
    coarsely while the overlap bumps get refined.
  * Every frame in a pair over `min_inliers` is a keyframe. Stopping at
    `min_stride` instead of 1 caps how finely the band is resolved, so keyframes
    land ~min_stride frames apart rather than one-per-frame (mostly redundant
    near-duplicates).
...
def select_keyframes(
    ...
    `max_stride` is the initial coarse grid (also the smallest overlap reliably
    detectable). `min_stride` is the finest level the band is refined to, so kept
    keyframes are spaced ~min_stride frames apart — raise it to thin further.
